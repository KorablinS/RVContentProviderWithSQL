...
import ru.synergy.rvcontentproviderwithsql.contentprovider.RequestProvider;
...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void fillTestElements() {
        ...
        for (int i = 0; i < cvArray.length; i++) {
            ...
            cv.put(TabbleItems.TEXT, ("text " + i));
            cvArray[i] = cv;
        }

        getContentResolver().bulkInsert(RequestProvider.uriForItems(0), cvArray);
    }

    private int getItemsCountLocal() {
        int itemsCount = 0;

        Cursor query = getContentResolver().query(RequestProvider.uriForItems(0), null, null, null, null);
        if (query != null) {
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        switch (id) {
            case 0:
                return new CursorLoader(this, RequestProvider.uriForItems(offset * page), null, null, null, null);
            default:
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        switch (loader.getId()) {
            case 0:
                ...
                MatrixCursor mx = new MatrixCursor(TabbleItems.Columns);
                fillMx(cursor, mx);
    ...
    private void fillMx(Cursor data, MatrixCursor mx) {
        ...
        while (data.moveToNext()) {
            mx.addRow(new Object[]{
                    data.getString(data.getColumnIndex(TabbleItems._ID)),
                    data.getString(data.getColumnIndex(TabbleItems.TEXT))
            });
        }
    }
